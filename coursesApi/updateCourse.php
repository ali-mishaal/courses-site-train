<?php
      ...
      $get_course = $obj->get_courses_information_by_topic_id($check_id);
      $get_topic = $obj->get_courses_information_by_type(1);
      $get_depart = $obj_depart->get_topic_information();
      $error = '';

      if (is_null($get_course)) {
      ...
      if (isset($_POST['title'])) {
          ...
          if ($get_course->title != $input_title ) {
              
              $update_title = $obj->update_courses($check_id , $input_title , NULL , NULL , 0 , 0 ,  0 , 0 , 
                -1 , -1);
              if (!$update_title) {
      ...
      if (isset($_POST['desc'])) {
          ...
          if($get_course->description != $input_desc){
      	       
               $update_desc = $obj->update_courses($check_id , NULL , $input_desc , NULL , 0 ,  0 , 0 , 0 , 
                -1 , -1);
      ...
      if (isset($_POST['type'])) {
          ...
          $input_depart = $_POST['specific_topic_belong'];
          $input_depart = $obj_depart->myTest($input_depart);
          $get_depart_id = $obj_depart->get_topic_information_by_name($input_depart);

      	  if ($input_type == 'Topic' || $input_type == 'topic') {
              $input_type = 1 ;
      	      $update_type = $obj->update_courses($check_id , NULL , NULL , NULL , 0 ,  0 , 0 , $input_type , $get_depart_id->id , 0);
      ...
      if (isset($_POST['courseBelong'])) {
      	  
      	  $input_courseBelong = $_POST['specific_course_belong'];
          $input_courseBelong = $obj->myTest($input_courseBelong);
      	  $get_topic_id = $obj->get_courses_information_by_name($input_courseBelong);
          
          if ($get_topic_id->id != $get_course->coursesBelongTo) {
              
              $update_belong = $obj->update_courses($check_id , NULL , NULL , NULL , 0 ,  0 , 0 , 0 , -1 , 
                                                  $get_topic_id->id);
  ...
  <div class='pop pop1'>
              <img src='<?php echo $get_course->image ?>'>
              <div class='cover_image'></div>

              <div class='word_web_de'>
                <h1><?php echo $get_course->title; ?></h1>
              </div>
                
              <div class='slide_web_de'>
                     <p> <?php echo $get_course->description ?></p>
	  ...
	  <section class="question_update">
         ...
         <div id='updateType'><P>Update<span> Type</span></P></div>
			   <div id='updateCourseBelong'><P>Update<span>BelongTo</span></P></div>
      ...
      <div id='Title' class='alll'>
       	       	                ...
       	       	                <form method="post" action="<?php  $_SERVER['PHP_SELF']; ?>">
                                          ...
                                          value="<?php echo $get_course->title; ?>">
         ...
         <div id='Type' class='alll'>
                             ...
                             <form method="post" action="<?php  $_SERVER['PHP_SELF']; ?>">
                                   ...
                                   <select name="specific_topic_belong" style="top: 51%;">
                                    <?php
			   ...
			   <div id='courseBelongTo' class='alll'>
			   	                   <h1>Update course Belong</h1>
			   	                   <span id="close5">&times;</span>
			   	                   <form method="post" action="<?php  $_SERVER['PHP_SELF']; ?>">
       	       	                	  <select name="specific_course_belong">
       	       	                	  	<?php
       	       	                	  	     for ($i=0; $i <count($get_topic) ; $i++) { 
       	       	                	  	     	 
                                             if ($get_course->courseBelongTo == $get_topic[$i]->id) {
                                               echo "<option selected>" . $get_topic[$i]->title . "</option>";
                                             }
       	       	                	  	     	 echo "<option>" . $get_topic[$i]->title . "</option>";
       	       	                	  	     }
       	       	                	 	 
       	       	                	 	?> 
       	       	                	 </select>
       	       	                	 <input type="submit" name="courseBelong" value="update">
       	       	                </form>
			   </div>
...
$(document).ready(function(){
      ...
      btn44.onclick = function() {
	    ...
	    var modal5 = document.getElementById('courseBelongTo');
        var btn5 = document.getElementById('updateCourseBelong');
