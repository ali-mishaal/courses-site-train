<section class="specilization">
                            
                      <div class="specilization_word"><h1>NEW COURSES</h1></div>

                      <?php 
                             $obj_topic = new coursesApi();

                             $obj_courses = new coursesApi();
                  $asc_courses = $conn->prepare("SELECT * FROM course WHERE type=0 ORDER BY created_at DESC");
                  $asc_courses->execute();
